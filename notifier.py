...
def _build_map_html(markers):
    """Build a Leaflet.js map with markers for each listing.
    ...
    return f"""
<h3>Map of today's listings</h3>
<p style='color:#666;font-size:12px'>Blue dots = rent &middot; Orange dots = sale.
Click a dot for details.</p>
<div id="map"></div>
<script>
(function() {{
...
def send(main_deals, still_active, comparison_html, status_note,
    ...
    html = build_html(main_deals, still_active, comparison_html, status_note,
                      recipient, price_data, map_markers)
    today = date.today().isoformat()
